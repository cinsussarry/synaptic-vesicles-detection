...
import numpy as np
from CNNs_GaussianNoiseAdder import MultiClass, GaussianNoiseAddition
...
# transform and load the training dataset
...
training_dataset = datasets.ImageFolder(root=os.path.join(
    'data_first_classifier', 'train'), transform=transform_train)
training_loader = torch.utils.data.DataLoader(dataset=training_dataset,
                                              batch_size=100, shuffle=True)

# transform and load the validation dataset
transform_val = transforms.Compose(
    [transforms.Resize((40, 40)),
     transforms.ToTensor(),
     transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
validation_dataset = datasets.ImageFolder(root=os.path.join(
    'data_first_classifier', 'val'), transform=transform_val)
...
print('Length validation dataset: ', len(validation_dataset))
...
for e in range(epochs):
    ...
    # iterate over the training_loader
    ...
    # now the validation dataset, no gradients calculation necessary
        ...
        with torch.no_grad():
            ...
            # iterate over the validation_loader
...
# print the results
    ...
    # here change accordingly '/path' to save the model weights where you want
    torch.save(model.state_dict(), os.path.join('/Users/Milagros/Downloads', 'model_dir',
                                                'epoch-{}.pth'.format(e)))
...
f1_val = []
for i in range(epochs):
    f1_val.append(2 * (val_precision_history[i] * val_recall_history[i]) /
